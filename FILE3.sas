...
/*********************************************************************************************************************************************
**GOALS
***1. Create analytic data set for mixed (per-prescription) and left-censored (tobit) per-clinician per-week model
******Files: 
*******a. mixed_analytic (per-prescription data for mixed model)
*******b. mixed_analyticu (per-prescription data for mixed model, included 'other' patients
*******c. analytic_tobit_18Aug25_A, analytic_tobit_18Aug25_B, analytic_tobit_18Aug25_AB, analytic_tobit_18Aug25_C, analytic_tobit_18Aug25_ABC 
*******d. secondary outcome, secondary_analytic_prop
***********************************************************************************************************************************************/
...
run;
%mend imp;
%imp(mixed_model_altamed_sample_18Aug25, alta);
%imp(mixed_model_altamed_sample_inc_undefined_18Aug25, altau);
...
proc sql;
	...
	(select prov_deid, clinic_id, pat_id, . as index_rx, week, total_mme, ln_total_mme, avg_daily_mme, ln_daily_mme, assignment, post, bpa length = 20, bpa_exp length = 20, inst, num_vsts from savepath.altau) 
	union all
	(select prov_deid, clinic_id, pat_id, index_rx as NU_prescription_id, week, total_mme, ln_total_mme, avg_daily_mme, ln_daily_mme, assignment, post, bpa length=20, bpa_exp length = 20, inst, num_vsts from bpa_type_v3);
quit;

*check number of patients by assignment, inst. and patient type;
proc sql;
	title "Clinician, patient, and clinic counts by assignment, institution and bpa type";
	select count(distinct pat_id) as pat_ct, count(distinct prov_deid) as prov_ct, count(distinct clinic_id), inst, assignment, bpa_exp from savepath.mixed_analyticu
	group by inst, assignment, bpa_exp
	order by inst, assignment, bpa_exp;
quit;
...
proc sql;
	...
	select count(distinct pat_id) as ct_pat, count(distinct prov_deid) as ct_prov, count(distinct clinic_id) as ct_clinic, BPA_exp from savepath.mixed_analytic
	group by BPA_exp
	order by BPA_exp;
quit;
...
proc sql;
	...
	select count(distinct pat_id) as ct_pat, count(distinct prov_deid) as ct_prov, count(distinct clinic_id) as ct_clinic, BPA_exp, assignment from savepath.mixed_analytic
	group by BPA_exp, assignment
	order by BPA_exp, assignment;
quit;
...
data all_weeks_v2;
	retain   
	ln_avg_total_mme inst PROV_DEID ASSIGNMENT CLINIC_ID WEEK POST;
	set all_weeks_v2;
run;
...
data savepath.analytic_tobit_18Aug25_&var;
	set analytic_tobit;
	by prov_deid week;
...
proc sql;
	...
	select count(distinct prov_deid) as ct_pres, count(distinct clinic_id) as ct_clinic from savepath.analytic_tobit_18Aug25_&var;
quit;

*# weeks per prescriber-should be 131;
proc sql;
	title "Check all clinicians have 131 weeks for tobit file";
	select count(*) as ct, prov_deid, inst from savepath.analytic_tobit_18Aug25_&var 
...
*tobit sample A + B;
data savepath.analytic_tobit_AB;
	set savepath.analytic_tobit_18Aug25_a
	savepath.analytic_tobit_18Aug25_b;
run;

*tobit sample A-C;
data savepath.analytic_tobit_ABC;
	set savepath.analytic_tobit_18Aug25_a
	savepath.analytic_tobit_18Aug25_b
	savepath.analytic_tobit_18Aug25_c;
run;

*combine BPAs;
%macro analytic(dat);
proc sql;
	create table savepath.analytic_tobit_18Aug25_&dat as 
	select	
...
proc sql;
	select count(distinct prov_deid) as ct_pres, count(distinct clinic_id) as ct_clinics, assignment from savepath.analytic_tobit_18Aug25_&dat
	group by assignment;
	title "Clinician and clinic counts by assignment data &dat";
quit;
...
*secondary outcome: proportion of patients who received a high dose Rx out of total patients per clinician-week
...
data temp;
	...
	wk = week - 27;
	if wk < 0 then kweek = 0;
	else kweek = wk;
run;

*output whether patient had high-dose Rx in given clinician-week;
data temp;
	set temp;
	if rn = 1 then output;
run;

*sum patients with high-dose prescription by clinician-week;
proc sql;
	...
	(sum(hi_dose_daily)/count(*)) as prop_week, (sum(hi_dose_daily)/count(*))*100 as perc from temp
